...
impl AgentLoop {
    ...
    pub async fn run(&mut self, mut rx: mpsc::Receiver<Envelope>) -> Result<()> {
        ...
        while let Some(env) = rx.recv().await {
            ...
            self.record_job_state(&env, JobState::Received, json!({"source": env.source, "content": env.content})).await;
            ...
            self.record_job_state(&env, JobState::Classified, json!({"intent": format!("{:?}", intent)})).await;

            match intent {
                Intent::Query => {
                    self.record_job_state(&env, JobState::Running, json!({"pipeline": "query"})).await;
                    let ctx = self.memory.retrieve_context(&env.content).await.unwrap_or_default();
                    info!("Query routed. Context chunks: {}", ctx.len());
                    self.record_job_state(&env, JobState::Completed, json!({"pipeline": "query", "context_chunks": ctx.len()})).await;
                }
                Intent::Command => {
                    self.record_job_state(&env, JobState::Running, json!({"pipeline": "command"})).await;
                    if env.content.trim() == "memory:compact" {
                        if let Err(e) = self.memory.compact_memory().await {
                            self.record_job_state(&env, JobState::Failed, json!({"pipeline": "command", "error": e.to_string()})).await;
                            continue;
                        }
                    }
                    info!("Command routed.");
                    self.record_job_state(&env, JobState::Completed, json!({"pipeline": "command"})).await;
                }
                Intent::AttackPayload => {
                    self.record_job_state(&env, JobState::Running, json!({"pipeline": "payload"})).await;
                    if let Some(name) = env.content.trim().strip_prefix("payload:") {
                        ...
                        if let Err(e) = self.security_policy.check_engagement_context("payload_execution") {
                            ...
                            self.record_job_state(&env, JobState::Denied, json!({"payload": name, "error": e.to_string()})).await;
                            continue;
                        }
                        ...
                        if !approved {
                            ...
                            self.record_job_state(&env, JobState::Denied, json!({"payload": name, "error": "ghostmcp denied"})).await;
                            continue;
                        }
                        ...
                        let _permit = match self.payload_slots.acquire().await {
                            Ok(p) => p,
                            Err(e) => {
                                self.record_job_state(&env, JobState::Failed, json!({"payload": name, "error": e.to_string()})).await;
                                continue;
                            }
                        };
                        ...
                        self.record_job_state(&env, JobState::Completed, json!({"payload": name})).await;
                    } else {
                        info!("AttackPayload intent but no payload directive. Ignored.");
                        self.record_job_state(&env, JobState::Ignored, json!({"reason": "missing payload directive"})).await;
                    }
                }
                Intent::Unknown => {
                    info!("Unknown intent. No action.");
                    self.record_job_state(&env, JobState::Ignored, json!({"reason": "unknown intent"})).await;
                }
            }
        }
